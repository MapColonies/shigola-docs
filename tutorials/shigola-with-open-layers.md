---
id: shigola-with-open-layers
title: "Shigola With OpenLayers"
sidebar_label: "Shigola With OpenLayers"
sidebar_position: 3
...
raster maps. Combined with Shigola, developers may now render maps on the web or
...
The first thing you'll need is a Shigola endpoint. You can either set up one
...
For Shigola, we'll be using the `VectorTile` source which is of type `MVT`.
The URL inside the source is the endpoint which will be queried to get the data.
In this case we are using a Shigola endpoint which serves up OpenStreetMap (OSM) data.
