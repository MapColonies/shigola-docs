...
date: 2026-08-17
linktitle: OGC API - Tiles
title: OGC API - Tiles
weight: 6
subtitle: A standards-compliant tile API alongside the native routes
menu:
  main:
    parent: Documentation
---

> Added by [this fork]({{< ref "/documentation/about-this-fork" >}}). Upstream Tegola serves its
> native `/maps/...` routes and a Mapbox TileJSON capabilities endpoint; it has no OGC API surface.

This fork serves [OGC API - Tiles](https://ogcapi.ogc.org/tiles/) for vector (Mapbox Vector Tile)
data. The surface is **additive**: the native
[`/maps/...` routes]({{< ref "/documentation/http-endpoints" >}}) keep serving the same tiles at the
same URLs, and a tile fetched either way is the same bytes from the same cache entry.

## Upgrading — two breaking changes

### 1. The viewer moved from `/` to `/viewer`

OGC API - Tiles requires a landing page at the service root, so the embedded viewer moved. `/viewer`
redirects to `/viewer/` — the viewer's assets are referenced relatively and only resolve from a URL
ending in a slash.

Update any bookmark, reverse-proxy rule or health check that pointed at `/`. A request for `/` now
returns the JSON landing page.
...
| `/tileMatrixSets` | The [tiling schemes]({{< ref "/documentation/tile-matrix-sets" >}}) served |
...
segments, and a dot or underscore can occur in a Tegola map or layer name, which would make the split
ambiguous.
...
the OGC conformance class. `pbf` is accepted because that is what the same tile is called by Tegola's
native routes, which serve it at a `.pbf` extension, and by the `format` member of the TileJSON —
being refused for using Tegola's own word for it would be surprising. Matching ignores case. The
...
it, and a Tegola map can declare query parameters that change what a tile contains. The native routes
...
conformance is verified with OGC's own [CITE](https://cite.opengeospatial.org/) suite. Tegola itself
is created and maintained by the [Go Spatial](https://github.com/go-spatial) team — see
