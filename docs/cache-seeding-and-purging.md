...
# Overview
The cache command manually manipulates Shigola's cache
...
> - [`--overwrite`](#cache-tiers) also **purges the tiers it does not write**.
>
> Note that `--bounds` landing exactly on a tile edge does not include the tile on the far side.
...
These subcommands are used to manipulate Shigola's cache.
...
> No effect on a single-backend cache.
...
Note: the Shigola server does not need to be running for this command to execute. However, your caching backend does.
...
The `purge` command is used to remove tiles from the cache. This can be used to remove outdated data, as Shigola prioritizes the cache.
...
* min-zoom -- If specified; Shigola will generate a range of tiles (from min-zoom to max-zoom (defaults to 22)) for each tile listed in the file.
* max-zoom -- If specified; Shigola will generate a range of tiles (from min-zoom (defaults to 0) to max-zoom) for each tile listed in the file.
...
The `tile-list` command instructs Shigola to read tile names from a file. The file is expected to have one tile per line, where each tile is formatted according to the format flag.
