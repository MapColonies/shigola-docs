...
sidebar_label: "Tile Matrix Sets"
sidebar_position: 5
description: "Serving more than one tiling scheme"
---
...
Shigola carries the OGC register of tile matrix sets as data, resolves grids through a registry, and
...
it is why the [cache key changed](./layered-cache.md) and why the tile row
...
`tile_matrix_sets` is set **per map**, not per layer or per provider. It is optional; a map that
omits it is served in every scheme this build supports.
